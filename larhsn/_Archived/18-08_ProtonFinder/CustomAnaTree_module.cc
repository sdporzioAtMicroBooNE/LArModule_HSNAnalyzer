#ifndef CustomAnaTree_module
#define CustomAnaTree_module

#include "AnaHelper.h"

// Analyzer class
class CustomAnaTree : public art::EDAnalyzer
{
public:
  explicit CustomAnaTree(fhicl::ParameterSet const & pset);
  virtual ~CustomAnaTree();
...
private:
  ...
  void ClearData();
}; // End class CustomAnaTree

CustomAnaTree::CustomAnaTree(fhicl::ParameterSet const & pset) :
    EDAnalyzer(pset),
    fNuLabel(pset.get<std::string>("nuLabel")),
    fCosmicLabel(pset.get<std::string>("cosmicLabel")),
    fVerbose(pset.get<bool>("verbose"))      
{} // END constructor CustomAnaTree

CustomAnaTree::~CustomAnaTree()
{} // END destructor CustomAnaTree

void CustomAnaTree::beginJob()
{
...
void CustomAnaTree::endJob()
{
} // END function endJob

void CustomAnaTree::ClearData()
{
  run = -1;
  subrun = -1;
  event = -1;
...
void CustomAnaTree::GetTruthInformation(art::Event const & evt)
{
...
void CustomAnaTree::analyze(art::Event const & evt)
{
...
DEFINE_ART_MODULE(CustomAnaTree)

#endif // END def CustomAnaTree_module